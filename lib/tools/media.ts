...
/** Minimal FFmpeg surface used by Converter / Compressor (avoids build-time package resolution issues). */
type FFmpeg = {
...
async function runFfmpegTransform(
  ...
  try {
    const { fetchFile } = (await new Function(
      'return import("@ffmpeg/util")'
    )()) as { fetchFile: (file: File | string) => Promise<Uint8Array> };
...
async function loadFfmpeg(
  ...
  ffmpegLoading = (async () => {
    ...
    const [{ FFmpeg }, { toBlobURL }] = (await Promise.all([
      new Function('return import("@ffmpeg/ffmpeg")')(),
      new Function('return import("@ffmpeg/util")')(),
    ])) as [
      { FFmpeg: new () => FFmpeg },
